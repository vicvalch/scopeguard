...
export async function resolveAuthorityChain(input: { workspaceId: string; delegationId: string; maxDepth?: number }) {
  const supabase = getAocAdapter("privilegedDb").createClient({ routeId: "governance.delegations.resolve_chain", operation: "resolve_delegation_chain", reason: "delegation_evaluation", systemActor: "system", workspaceId: input.workspaceId });
...
export async function evaluateDelegatedAccess(input: DelegationInput & { delegationId?: string; delegationToken?: string }) {
  const validated = await validateDelegatedCapability(input);
...
export async function revokeDelegatedCapability(input: DelegationInput) { const supabase = getAocAdapter("privilegedDb").createClient({ routeId: "governance.delegations.revoke", operation: "revoke_delegation", reason: "delegation_revocation", systemActor: "system", workspaceId: input.workspaceId, actorUserId: input.actorUserId ?? null }); const now = new Date().toISOString(); const { data } = await supabase.from("governance_delegations").update({ status: "revoked", revoked_at: now, revoked_reason: input.revokedReason ?? "manual_revoke", updated_at: now }).eq("id", input.delegationId).select("*").maybeSingle(); if (!data) return { ok: false, reason: "not_found" }; const { data: descendants } = await supabase.from("governance_delegations").select("id,parent_delegation_id,workspace_id,project_id,requested_permission").eq("workspace_id", data.workspace_id);
  ...
  await getAocAdapter("securityAudit").logEvent("delegated_capability_revoked", { workspaceId: data.workspace_id, projectId: data.project_id, actorUserId: input.actorUserId ?? null, requested_permission: data.requested_permission, metadata: { delegationId: data.id, propagatedRevocations: childIds.length } }); return { ok: true, delegation: data, propagatedRevocations: childIds.length }; }


export async function consumeDelegatedCapability(input: DelegationInput) {
  const evaluated = await evaluateDelegatedAccess(input);
  if (!evaluated.allowed) return { ok: false, reason: String(evaluated.decision), delegation: evaluated.delegation };
  const supabase = getAocAdapter("privilegedDb").createClient({ routeId: "governance.delegations.consume", operation: "consume_delegation", reason: "delegated_execution", systemActor: "system", workspaceId: input.workspaceId, actorUserId: input.actorUserId ?? null });
