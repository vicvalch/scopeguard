...
export async function POST(request: Request) {
  ...
  const body = await request.json() as { form: Record<string, string> & { storageStrategy?: "cloud" | "local" | "self_hosted" }; templates: TemplateInput[]; loadSample?: boolean };
  const supabase = await createSupabaseServerClient();

  const projectName = body.loadSample ? "PMFreak Sample Launch Recovery" : body.form.projectName || "First Activated Project";
  const description = body.loadSample
  ...
  const demoAppend: TemplateInput[] = body.loadSample ? [
  ...
  for (const t of [...body.templates, ...demoAppend]) {
    await saveOperationalMemory({ companyId: user.companyId, projectId: project.id, domain: t.domain, title: t.title, text: t.text, sourceRef: body.loadSample ? "activation-sample" : "activation" });
  }
