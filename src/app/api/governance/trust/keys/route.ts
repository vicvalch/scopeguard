...
type SigningKeyRow = {
  ...
  capability_trust_domains: { domain_key: string; verification_mode: string | null } | null;
};
...
export async function GET() {
  ...
  const keys = (data ?? []).map((k: SigningKeyRow) => ({ trustDomain: k.capability_trust_domains?.domain_key ?? null, keyId: k.key_id, algorithm: k.algorithm, status: k.status, validFrom: k.valid_from, validUntil: k.valid_until, verificationMode: k.capability_trust_domains?.verification_mode ?? null, publicJwk: k.algorithm === "Ed25519" ? (k.public_metadata?.public_key_jwk ?? null) : null, publicPem: k.algorithm === "Ed25519" ? (k.public_metadata?.public_key_pem ?? null) : null }));
