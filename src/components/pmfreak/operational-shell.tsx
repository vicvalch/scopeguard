      ...
      <div className="mx-auto flex w-full max-w-7xl gap-6 px-4 py-5 md:px-8 md:py-8">
        <aside className="sticky top-6 hidden h-[calc(100vh-3rem)] w-72 flex-col rounded-3xl border border-white/10 bg-white/5 p-5 backdrop-blur-xl lg:flex">
          ...
          <nav className="mt-6 space-y-2" aria-label="Primary">
            {PM_MODULES.map((item) => (
              <Link key={item.href} href={navHref(item.href)} className={`block rounded-xl border px-4 py-3 transition ${pathname.startsWith(item.href) ? "border-cyan-300/50 bg-cyan-300/10" : "border-transparent hover:border-cyan-300/40 hover:bg-cyan-300/10"}`}>
          ...
          <div className="mt-auto rounded-2xl border border-white/10 bg-white/20 p-4 text-xs text-slate-300">
            ...
            <p>{user.role}</p>
            <Link href="/logout" className="mt-3 inline-block text-cyan-200">Logout</Link>
          </div>
        </aside>
        ...
        <div className="flex-1 space-y-4">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="mb-4 grid gap-2 lg:hidden">
              <p className="text-xs uppercase tracking-[0.2em] text-cyan-300">Navigate</p>
              <div className="flex snap-x gap-2 overflow-x-auto pb-1">
                {PM_MODULES.map((item) => (
                  <Link key={item.href} href={navHref(item.href)} className={`shrink-0 rounded-lg border px-3 py-2 text-xs ${pathname.startsWith(item.href) ? "border-cyan-300/50 bg-cyan-300/10" : "border-white/15 bg-white/10"}`}>
                    {item.label}
                  </Link>
                ))}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-xs uppercase tracking-[0.2em] text-cyan-300">Project Scope</span>
              <select value={projectId} onChange={(e) => setProjectId(e.target.value)} className="rounded-lg border border-white/10 bg-white/30 px-3 py-1 text-sm" disabled={projectsLoading} aria-label="Select project scope">
            ...
            {projectsLoading ? <p className="mt-2 text-xs text-slate-400">Loading projects…</p> : null}
            {projectsError ? <p className="mt-2 text-xs text-amber-200">{projectsError}</p> : null}
            {!projectsLoading && !projectsError && projects.length === 0 ? (
              <p className="mt-2 text-xs text-cyan-200">No projects yet. Create your first project in the Projects module.</p>
            ) : null}

            <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-300">
              {OPERATIONAL_FLOW.map((step, idx) => (
                <span key={step}>{step}{idx < OPERATIONAL_FLOW.length - 1 ? " →" : ""}</span>
              ))}
            </div>
          </div>
